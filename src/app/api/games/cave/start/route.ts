...
export async function POST(req: NextRequest) {
  ...
  const existing = await userActiveGame(user.id, "cave");
  if (existing) return err("You already have an active Cave game", 409);
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      await tx.walletLedger.create({
        data: {
          ...
          transactionType: "BET", note: `Cave bet`,
        },
      });
